...
function addToFeatureVector($tweet, $stopWords, $subject=null) {
	$featureVector = array();
	$words = explode(" ", $tweet);
    //load the subject words into an array.  For classifying tweets this is the search phrase.  Need to make sure that the word isn't part
    //of the search phrase as it distorts results.
   	$wordsInSubject = explode(" ", strtolower($subject)) ;
...
//		$w= preg_replace('/#([^\s]*)/', '$1', $w) ;
		...
		if (in_array($w, $wordsInSubject, true)) {
   			continue ;
		}
